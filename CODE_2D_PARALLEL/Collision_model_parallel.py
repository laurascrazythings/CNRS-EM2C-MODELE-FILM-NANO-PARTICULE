...
for t in range(1, Nt+ 1):   
    ...
    if len(Index_par_local) > 0 or len(Index_par_ghost_a) > 0 or len(Index_par_ghost_b) > 0 :
        ...
        for par in reversed(range(len(Index_par_local))):
            ...
            elif (XY_local_update[Index, 0] < (Local_end - Buffer_zone_width[0]) and Local_sent_next[Index]): #if it leaves on the other side back to false
